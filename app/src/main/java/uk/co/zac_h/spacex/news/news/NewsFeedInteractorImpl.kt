...
import uk.co.zac_h.spacex.rest.NewsInterface
import java.net.UnknownHostException
import kotlin.coroutines.CoroutineContext

class NewsFeedInteractorImpl : NewsFeedInteractor {
    ...
    override fun getNews(listener: NewsFeedInteractor.Callback) {
        scope.launch {
            val response = async(SupervisorJob(parentJob)) {
                NewsInterface.create().getNewsFeed("SpaceX", "rss")
            }

            withContext(Dispatchers.Main) {
                try {
                    if (response.await().isSuccessful) {
                        listener.onSuccess(response.await().body()?.channel?.articleList)
                    } else {
                    ...
                    Log.e(
                        this@NewsFeedInteractorImpl.javaClass.name,
